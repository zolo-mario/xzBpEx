...
def add_variable_to_blueprint(variable_name: str, variable_type: str, default_value: str = None, category: str = None, blueprint_path: str = None) -> str:
    ...
    Args:
        ...
        category (str, optional): The category name to organize the variable in MyBlueprint panel.
        blueprint_path (str, optional): The path to the target Blueprint. If omitted, uses the selected asset.
...
def how_to_use() -> str:
    """
    Call this first in a new session. It returns the master prompt that explains all available filesystem tools.
    """
    try:
        # This will correctly find the file inside the extension package
        current_dir = Path(__file__).parent
        md_file_path = current_dir / "filesystem_how_to_use.md" # <-- This loads the small, specific file
        if not md_file_path.exists():
...
def read_file_contents(file_path: str) -> str:
    ...
    try:
        ...
        # Use the helper for consistent, safe encoding handling
...
def write_to_file(file_path: str, content: str, append: bool = False) -> str:
    ...
    Args:
        ...
        append (bool, optional): If True, appends the content to the end of the file. 
...
# ==================================================================
# MAIN SCRIPT EXECUTION (FIXED LOGIC)
# ==================================================================
if __name__ == "__main__":
    # --- THIS IS THE NEW LOGIC ---
    parser = argparse.ArgumentParser(description="Run a specific MCP server.")
    parser.add_argument('--server-name', required=True, help="The name of the server to run ('unreal-handshake' or 'filesystem-access').")
    args = parser.parse_args()

    print(f"Python MCP script started. Launching server: '{args.server_name}'", file=sys.stderr)
    
    if args.server_name == "unreal-handshake":
        mcp.run()
    elif args.server_name == "filesystem-access":
        fs_mcp.run()
    else:
        print(f"Error: Unknown server name '{args.server_name}'. Exiting.", file=sys.stderr)
        sys.exit(1)